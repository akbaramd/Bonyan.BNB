...
public static class BnbCollectionExtensions
  {
    /// <summary>
    ...
    public static IEnumerable<T> AddIfNotContains<T>(
      ...
      IEnumerable<T> items)
    {
      BnbCheck.NotNull<ICollection<T>>(source, nameof (source));
    ...
    {
      BnbCheck.NotNull<ICollection<T>>(source, nameof (source));
      BnbCheck.NotNull<Func<T, bool>>(predicate, nameof (predicate));
      BnbCheck.NotNull<Func<T>>(itemFactory, nameof (itemFactory));
